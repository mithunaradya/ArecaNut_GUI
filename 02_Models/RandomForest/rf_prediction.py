...
class ImageClassifierApp(tk.Tk):
    def __init__(self):
        ...
        self.rf_classifier = self.load_classifier('rf_classifier_model.pkl')
    ...
    def create_widgets(self):
        ...
        self.label_result_rf = tk.Label(self, text="")
        self.label_result_rf.pack(pady=5)
    ...
    def select_image(self):
        ...
        if image_path:
            ...
            rf_prediction = self.predict_rf(image_path)
            self.label_result_rf.config(text=f"RF Prediction: {rf_prediction}")
    ...
    def predict_rf(self, image_path):
        image = self.preprocess_image(image_path)
        features = self.extract_hsv_features(image)
        features = np.array(features).reshape(1, -1)
        predicted_label = self.rf_classifier.predict(features)
